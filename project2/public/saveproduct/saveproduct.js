                ...
                success: (userdata) => {
                    for (let i = 0; i < userdata.length; i++) {
                        if (userdata[i].email == localStorage.getItem('user')) {
                            console.log(userdata[i].saveproduct)
                            var list = "";
                            for (let j = 0; j < userdata[i].saveproduct.length; j++) {
                                console.log(userdata[i].saveproduct[j]);
                                for (let k = 0; k < data.length; k++) {
                                    if (userdata[i].saveproduct[j] == data[k]._id) {
                                        ...
                                        <div class="row" style="background-color: whitesmoke; border-radius: 4px; height: 150px;">
                                            <div class="col-1" style="border-right: 1px solid gray">
                                                <div class="row" style="margin-top:30px; margin-left:15px;">
                                                    <a href="http://localhost:3000/removesaveproduct/${userdata[i]._id}/${data[k]._id}"><i class="fa fa-times fa-2x" aria-hidden="true"></i></a>
                                                </div>
